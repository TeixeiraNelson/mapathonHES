...
Constant variables used in this part of the program, that's why they are global.
 => 2 Created icons and 1 BaseLayer for the maps' marker layers
 */
...
/*
App component, displays the whole application.
 */
class AppComponent extends React.Component {
    /*
    Constructor with a lot of variables, has the complexity of the application grew we had to add some.
     */
    constructor(props) {
        super(props);
        this.nbClicks = 0;
    ...
    /*
    Function that allows to open or close the side bar
     */
    ...
    addMarkerToMap = () => {
        if (this.state.addMarkerEnabled === false)
            this.setState({addMarkerEnabled: true});

        this.onSetSidebarOpen(false);
        alert("Click on the map to add a P.O.I at your click's position.")
    };

    /*
    Function called by the Popup button to delete a poi,
    ...
    /*
    Function that updates the map after a P.O.I has been deleted in the database.
     */
    updateMap = (data) => {
    ...
    if the marker has already been created, navigates to user to the point.
     */
    localiseUser = () => {
    ...
    /*
    Function that adds the marker to the map wherever the user clicked on it.
     */
    addMarker = e => {
    ...
    /*
    function that generates the group markers of the layers that we can filter, based on which parameter we want (example : group number)
     */
    generateGroupMarkers(number) {
        ...
        if (number === 0) {
            ...
            this.state.markers.map((position) => {
                if (typeof position.Creator !== 'undefined' && position.Creator !== null && position.Creator.group === number)
                    filteredMarkers.push(position)
            })
        } else {
            this.state.markers.map((position) => {
                if (typeof position.Creator !== 'undefined' && position.Creator.id === this.state.currentUser.sub)
                    filteredMarkers.push(position)
            })
        }
    ...
    /*
    Formats a P.O.I date to be easier to read
     */
   ...
   ;
    ...
    /*
    Method that handles the way a P.O.I is modified.
     */
    modifyPoi = (position) => {
        this.onSetSidebarOpen(true);
        this.setState({modifyingMarker: true});
